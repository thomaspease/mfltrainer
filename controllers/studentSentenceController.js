...
exports.createStudentSentencesDuringTraining = catchAsync(
  async (req, res, next) => {
    const studentTask = await StudentTask.findById(req.params.id);
    const task = await Task.findById(studentTask.task);

    const student = studentTask.user;
    ...
    const alreadyInserted = (
      await StudentSentence.find({
        student,
        exercise,
        sentence: { $in: sentenceIds },
      }).exec()
    ).map((doc) => doc.sentence.toString());

    const retestDays = 1;
    const retestOn = Date.now() + retestDays * 24 * 60 * 60 * 1000;

    const newDocs = sentenceIds
      .filter((id) => !alreadyInserted.includes(id))
      .map((id) => {
        return {
          sentence: id,
          student,
